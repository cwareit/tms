...
@section('title')
    नयाँ बैङ्क | {{$school->name}} | {{ config('app.name') }}
@endsection
...
<nav aria-label="breadcrumb">
  <ol class="breadcrumb">
    ...
    <li class="breadcrumb-item active" aria-current="page">नयाँ बैङ्क</li>
  </ol>
</nav>
...
<div class="card">
    <div class="card-header text-center">
        <h4>नयाँ बैङ्क | {{$school->name}} | {{ config('app.name') }}</h4>
    </div>
    <div class="card-body">
    ...
    <small>
        ...
        <form method="post" action="{{route('new_bank', ['school_id' => $school->id])}}">
            @csrf


            <div class="mb-3">
                <label for="name" class="form-label">नाम  <span class="text-danger fw-bold">*</span></label>
                <input type="string" class="form-control" id="name" name="name" value="{{old('name')}}">
                @error('name')
            ...
            <div class="mb-3">
                ...
                <input type="string" class="form-control" id="branch" name="branch" value="{{old('branch')}}">
                @error('branch')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="string" class="form-control" id="account_number" name="account_number" value="{{old('account_number')}}">
